...
def get_portfolio_data():
    """Calculates live portfolio performance with inception correction."""
    try:
        ...
        start_date = min(first_purchase, current_year_start)
        ...
        for date, prices in market_data.iterrows():
            ...
            daily_invested = (active_holdings['amount'] * active_holdings['price']).sum()
...
if st.session_state.page == 'landing':
    ...
    st.markdown("### Welcome / Üdvözöljük")
    st.markdown("Please select an investment category to view performance.")
    
    spacer, btn_col1, btn_col2, spacer2 = st.columns([1, 3, 3, 1])
    
    with btn_col1:
        st.info("### 📈 Alternative Investing")
        st.write("Quantitative strategies including USEquity 500 and legacy models.")
        if st.button("Enter Alternative Section", use_container_width=True):
            go_to_alternative()
            st.rerun()

    with btn_col2:
        st.success("### 💼 Equity Portfolio")
        st.write("Medium risk equity fund with live performance tracking.")
    ...
    st.sidebar.divider()
    st.sidebar.header(_("fund_selection"))
    st.sidebar.selectbox("Language / Nyelv", options=list(language_options.keys()), key='lang_selector', on_change=set_language)

    fund_list = ["USEquity500", "IBUST (Inactive)"]
    selected_fund = st.sidebar.selectbox(_("select_fund"), fund_list)
    ...
    if nav_df is not None and holdings is not None:
        ...
        display_df = holdings[['ticker', 'amount', 'cost_basis_total', 'current_price', 'market_value', 'unrealized_pnl', 'weight', 'return_pct']].copy()
        ...
        st.dataframe(
            ...
            column_config={
                "ticker": "Ticker",
                "amount": "Qty",
                "cost_basis_total": st.column_config.NumberColumn("Total Cost", format="$%.2f"),
